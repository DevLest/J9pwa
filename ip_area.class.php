...
class ip_area {
	...
	public function __construct() {
		...
		$qqdatasrc =  PC_PATH.'iparea'.DIRECTORY_SEPARATOR.'QQWry.DAT';
		if(@file_exists(PC_PATH.'iparea'.DIRECTORY_SEPARATOR.'mini.Dat')) {
			$this->func = 'data_mini';
			$this->fp = @fopen(PC_PATH.DIRECTORY_SEPARATOR.'iparea'.DIRECTORY_SEPARATOR.'mini.Dat', 'rb');
			$this->offset = unpack('Nlen', fread($this->fp, 4));
			$this->index  = fread($this->fp, $this->offset['len'] - 4);
		} elseif(@file_exists($qqdatasrc)) {
			$this->func = 'data_full';
			$this->fp = @fopen($qqdatasrc, 'rb');
		}
	}
